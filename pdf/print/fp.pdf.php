...
/*****************************************************************************************/
		...
		$query = "SELECT codeart, designation, qte, prix_org FROM fps_liste WHERE num_piece='$code' ";
		...
		while( $row = mysql_fetch_assoc($rows) ){
            ...
            //totaux
            $this->SetFillColor(255,255,255);
            $query = "SELECT    fps.date as fps_date,
                                ...
                                taxe_supp
                            FROM fps 
                            LEFT JOIN clients ON fps.codeclient=clients.codeclient 
                            WHERE fps.code='$code' ";
            ...
            while( $row = mysql_fetch_assoc($rows) ){
                $net_ht    = $row['total_ht'];
                $total_ht  = $row['total_ht'] + $row['remise'];
                ...
                $this->Cell($w[4], 8, "Arrêter la présente facture proformat à la somme de : ", '', 0, 'L', $fill);
...
$pdf->SetTitle("Facture Proformat");
...
$html = "<br><br><br><br><table><tr><td></td><td><b>Devis N° " . $code . "</b></td><td>" . "</td></tr></table><br><br><br><br>";
