...
/*****************************************************************************************/
		...
		$query = "SELECT codeart, designation, qte, prix_org FROM cmds_clients WHERE num_piece='$code' ";
		...
		while( $row = mysql_fetch_assoc($rows) ){
            ...
            //totaux
            $this->SetFillColor(255,255,255);
            $query = "SELECT    ccs.date as ccs_date,
                                ...
                                ccs.taxe_supp
                            FROM ccs 
                            LEFT JOIN clients ON ccs.codeclient=clients.codeclient
                            WHERE ccs.code='$code' ";
            $rows = mysql_query($query);
            ...
            while( $row = mysql_fetch_assoc($rows) ){
                ...
                $query = "SELECT    reglements_pieces.id,
                            ...
                            FROM ccs 		
                            LEFT JOIN reglements_pieces ON ccs.code=reglements_pieces.num_piece
                            LEFT JOIN reglements        ON reglements.code=reglements_pieces.num_reg
                            WHERE ccs.code='$code' 
                                AND reglements_pieces.piece='CC'";
                $rows = mysql_query($query);
                ...
                $this->Cell($w[4], 8, "Arrêter la présente commande à la somme de : ", '', 0, 'L', $fill);
...
$pdf->SetFont($police, '', 13);
$html = "<br><br><br><br><table><tr><td></td><td><b>Commande client N° " . $code . "</b></td><td>" . "</td></tr></table><br><br><br><br>";
...
$query = "SELECT    ccs.date as ccs_date,
                    ...
                    timbre
                FROM ccs 
                LEFT JOIN fournisseurs ON ccs.codefour = fournisseurs.codefour 
                WHERE ccs.code='$code' ";
