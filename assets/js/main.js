...
var App = function() {
    ...
    App = function() {
        ...
        return App.dashboard = function() {
			...
			/* Top Articles */	
            function c() {
				//get top 5 articles with 'designation' & 'total_ht'
				$.ajax({
					  url  : "snippets/top5Articles.php" ,
					  type : "POST",
					  dataType : "json",
					  data : {},
					  success: function(data,status,xhr){  
							/*for( var i=0; i<= data.length ; i++ ){
								labels[i] = data[i].designation ;			
								datas[i]  = data[i].sum ;					
							}*/
							var a = [{
									label: data[0].designation,
									data: data[0].sum
								}, {
									label: data[1].designation,
									data: data[1].sum
								}, {
									label: data[2].designation,
									data: data[2].sum
								}, {
									label: data[3].designation,
									data: data[3].sum
								}, {
									label: data[4].designation,
									data: data[4].sum
								}],
								b = App.color.alt2,
								c = App.color.alt4,
            ...
            function f() {
				...
				$.ajax({
					  url  : "snippets/NbCmdsParMois.php",
					  type : "POST",
					  dataType : "json",
					  data : {},
					  success: function(data,status,xhr){
								...
								for(var i=0; i<data.length; i++){
									sums[i] = new Array(i+1, parseInt(data[i].sum) );
									mois[i] = data[i].mois;
								}
								
								
										var a = ($("#line-chart1"), 				
													//[  [1, 100],[2, 150],[3, 55],[4, 36],[5, 57],[6, 80],[7, 65]  ]
													sums
												),
											b = App.color.alt3;
										$.plot("#line-chart1", [{
            ...
            function j() {				
				$.ajax({
					  url  : "snippets/salesLast7Months.php",
					  type : "POST",
					  dataType : "json",
					  data : {},
					  success: function(data,status,xhr){   
								...
								var a = tinycolor(App.color.primary).lighten(6),
									...
									c = {
										...
										labels : mois,
										datasets: [{
											label: "Ventes des derniers mois",
											...
											//data: [133, 122, 111, 144, 155, 166, 177]
											data : sums
										}]
									};
